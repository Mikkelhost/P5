...
/*******************************************************************************
* Function Name: Clock_aclk_Start
********************************************************************************
*
* Summary:
...
void Clock_aclk_Start(void) 
{
    ...
    Clock_aclk_CLKEN |= Clock_aclk_CLKEN_MASK;
	Clock_aclk_CLKSTBY |= Clock_aclk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_aclk_Stop
********************************************************************************
*
* Summary:
...
void Clock_aclk_Stop(void) 
{
    ...
    Clock_aclk_CLKEN &= (uint8)(~Clock_aclk_CLKEN_MASK);
	Clock_aclk_CLKSTBY &= (uint8)(~Clock_aclk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_aclk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_aclk_CLKEN & Clock_aclk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_aclk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_aclk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_aclk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_aclk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_aclk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_aclk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_aclk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_aclk_GetSourceRegister();
    uint16 oldDivider = Clock_aclk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_aclk_CLKEN & Clock_aclk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_aclk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_aclk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_aclk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_aclk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_aclk_MOD_SRC |= modeBitMask & (uint8)Clock_aclk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_aclk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_aclk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_aclk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_aclk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_aclk_MOD_SRC & (uint8)(Clock_aclk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_aclk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_aclk_GetDividerRegister();
    uint8 oldSrc = Clock_aclk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_aclk_MOD_SRC |= CYCLK_SSS;
        Clock_aclk_MOD_SRC =
            (Clock_aclk_MOD_SRC & (uint8)(~Clock_aclk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_aclk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_aclk_MOD_SRC & Clock_aclk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_aclk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_aclk_PHASE = clkPhase & Clock_aclk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_aclk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_aclk_PHASE & Clock_aclk_PHASE_MASK;
}
